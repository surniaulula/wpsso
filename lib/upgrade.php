	...
	class WpssoOptionsUpgrade extends WpssoOptions {
		...
		public function options( $options_name, &$opts = array(), $def_opts = array(), $network = false ) {
			...
			if ( $options_name === constant( 'WPSSO_OPTIONS_NAME' ) ) {
				...
				if ( $prev_version > 0 && $prev_version <= 548 ) {
					if ( is_admin() ) {
						...
						$this->p->notice->upd(
							sprintf( __( 'This latest version of %1$s includes a feature to monitor the WordPress content filter and fix incorrectly coded filter hooks (that send text to the webpage output instead of returning it, as all filter hooks must).', 'wpsso' ), $short_name ).' '.
