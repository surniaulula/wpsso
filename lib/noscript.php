	...
	class WpssoNoScript {
		...
		public function get_single_image( array $mod, &$mixed, $mt_pre = 'og:image' ) {
			...
			if ( empty( $mixed ) ) {
				...
				$image_url = SucomUtil::get_mt_media_url( $mixed, $mt_pre );

				if ( empty( $image_url ) ) {
					if ( $this->p->debug->enabled ) {
						$this->p->debug->log( 'exiting early: ' . $mt_pre . ' url values are empty' );
					}
					return array();
				}
				...
				$mt_image = array_merge(
					...
					( empty( $mixed[ $mt_pre . ':width' ] ) ? array() : $this->p->head->get_single_mt( 'meta',	// Meta itemprop.
						'itemprop', 'image.width', $mixed[ $mt_pre . ':width' ], '', $mod ) ),
					( empty( $mixed[ $mt_pre . ':height' ] ) ? array() : $this->p->head->get_single_mt( 'meta',	// Meta itemprop.
						'itemprop', 'image.height', $mixed[ $mt_pre . ':height' ], '', $mod ) )
				);
