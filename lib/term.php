	...
	class WpssoTerm extends WpssoMeta {
		...
		public function get_posts( array $mod, $posts_per_page = false, $paged = false ) {
			...
			if ( $max_time > 0 && $total_time > $max_time ) {
				...
				// translators: %1$0.3f is a number of seconds, %2$s is the sub-system name, %3$d is an ID number, %4$s is a taxonomy name, %5$0.3f is a number of seconds
				$error_msg = sprintf( __( 'slow query detected - WordPress get_posts() took %1$0.3f secs to get posts for %2$s id %3$d in taxonomy %4$s (longer than recommended max of %5$0.3f secs)', 'wpsso' ), $total_time, $mod['name'], $mod['id'], $mod['tax_slug'], $max_time );
