	...
	class WpssoPost extends WpssoMeta {
		...
		public function get_posts( array $mod, $posts_per_page = false, $paged = false ) {
			...
			if ( $max_time > 0 && $total_time > $max_time ) {
				...
				$error_msg = sprintf( __( 'slow query detected - WordPress get_posts() took %0.4f secs to get the children of %s id %d (longer than recommended max of %0.4f secs)', 'wpsso' ), $total_time, $mod['name'], $mod['id'], $max_time );
			...
			if ( $this->p->debug->enabled ) {
				$this->p->debug->log( count( $post_posts ).' post objects returned in '.sprintf( '%0.4f secs', $total_time ) );
			}
