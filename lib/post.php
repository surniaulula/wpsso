						...
						foreach ( $scripts_json as $script_md5 => $single_json ) {
							...
							if ( strpos( $single_json, $id_marker ) ) {
								...
								if ( $is_admin ) {
									...
									$notice_msg = sprintf( __( 'The webpage at %1$s includes basic and incomplete Schema JSON-LD product markup from the WooCommerce plugin.', 'wpsso' ), '<a href="' . $check_url . '">' . $check_url_htmlenc . '</a>' ) . ' ';
