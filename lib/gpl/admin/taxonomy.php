...
if ( ! class_exists( 'WpssoGplAdminTaxonomy' ) ) {
	...
	class WpssoGplAdminTaxonomy {
		...
		public function filter_taxonomy_header_rows( $rows, $form, $head_info ) {
			...
			$rows[] = $this->p->util->th( 'Default Title', 'medium', 'meta-og_title', $head_info ). 
			...
			$this->p->util->th( 'Google+ / Schema Description', 'medium', 'meta-schema_desc', $head_info ).
			'<td class="blank">'.$this->p->webpage->get_description( $this->p->options['schema_desc_len'], '...', 
				false ).'</td>';	// use_post = false
	
			$rows[] = $this->p->util->th( 'Google Search / SEO Description', 'medium', 'meta-seo_desc', $head_info ).
			...
			$rows[] = $this->p->util->th( 'Twitter Card Description', 'medium', 'meta-tc_desc', $head_info ).
		...
		public function filter_taxonomy_media_rows( $rows, $form, $head_info ) {
			...
			$rows[] = '<tr class="hide_in_basic">'.
			$this->p->util->th( 'Image Dimensions', 'medium', 'og_img_dimensions' ).
			'<td class="blank">'.$form->get_image_dimensions_text( 'og_img', true ).'</td>';

			$rows[] = $this->p->util->th( 'Image ID', 'medium', 'meta-og_img_id', $head_info ).
			'<td class="blank">&nbsp;</td>';

			$rows[] = $this->p->util->th( 'or Image URL', 'medium', 'meta-og_img_url', $head_info ).
			'<td class="blank">&nbsp;</td>';

			$rows[] = $this->p->util->th( 'Video Embed HTML', 'medium', 'meta-og_vid_embed', $head_info ).
			'<td class="blank">&nbsp;</td>';

			$rows[] = $this->p->util->th( 'or Video URL', 'medium', 'meta-og_vid_url', $head_info ).
			'<td class="blank">&nbsp;</td>';
			...
			$rows[] = '<tr class="hide_in_basic">'.
			$this->p->util->th( 'Image ID', 'medium', 'meta-rp_img_id', $head_info ).
			'<td class="blank">&nbsp;</td>';

			$rows[] = '<tr class="hide_in_basic">'.
			$this->p->util->th( 'or Image URL', 'medium', 'meta-rp_img_url', $head_info ).
			'<td class="blank">&nbsp;</td>';
