...
if ( ! class_exists( 'WpssoMetaItem' ) ) {
	...
	class WpssoMetaItem {
		...
		public function get_array( array $mod, array $mt_og = array() ) {
			 ...
			 */
			if ( ! apply_filters( 'wpsso_add_schema_meta_array', true ) ) {
			...
			if ( empty( $this->p->options[ 'add_link_itemprop_thumbnailurl' ] ) ) {
				...
				$mt_item[ 'thumbnailurl' ] = $this->p->og->get_thumbnail_url( 'wpsso-thumbnail', $mod, $md_pre = 'schema' );
			...
			return (array) apply_filters( 'wpsso_schema_meta_itemprop', $mt_item, $mod, $mt_og, $page_type_id );
		}
	}
}
