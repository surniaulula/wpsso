...
(2020/05/30) Added a "Copy notifications to clipboard." link in the SSO notifications list. Added an informational message about Schema FAQPage, QAPage, and Question markup when selecting those types. Fix to skip URL shortening for 404 and search pages.

= 7.6.0 =

(2020/05/22) Added post cache clearing for attached post IDs. Added a new "Show Yoast SEO Import Details" option. Removed the "Include Schema WebSite" option. Moved the 'sucom-block-editor-admin' script to work around a bug in NextGEN Gallery.

= 7.5.0 =

(2020/05/14) Added a check to prevent post URLs from non-public post types from being shortened. Fixed an ignored Product Availability value in the Document SSO metabox for custom product pages that are not e-commerce product pages.
